...
export async function managementRoutes(app:FastifyInstance,db:PrismaClient,env:Env,broadcast:()=>Promise<void>){const {requireAdmin}=authTools(db,env);
 ...
 for(const method of ['POST','PATCH'] as const)app.route({method,url:'/api/admin/agents'+(method==='PATCH'?'/:id':''),preHandler:requireAdmin,handler:async req=>{const input=agentSchema.parse(req.body);const id=method==='PATCH'?idParam.parse(req.params).id:undefined;const previous=id?await db.agent.findUnique({where:{id},select:{avatarUrl:true}}):null;const result=await serial(db,async tx=>{if(input.teamId&&!await tx.team.findFirst({where:{id:input.teamId,active:true}}))throw new ApiError(400,'INVALID_TEAM','Choose an active team.');const data={...input,employeeCode:input.employeeCode||null,email:input.email||null,avatarUrl:input.avatarUrl||null};return id?tx.agent.update({where:{id},data}):tx.agent.create({data});});if(previous&&previous.avatarUrl!==result.avatarUrl)await deleteUploadedAvatar(previous.avatarUrl);await broadcast();return result;}});
